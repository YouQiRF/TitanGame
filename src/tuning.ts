...
export const T = {
  ...
  puppeteer: {
    ...
    execute: {
      ...
      /**
       * 行星系統：她跳到場地**最右邊**，玩家被推到**最左邊**，
       * 她的球化成一圈圈同心軌道繞著她公轉（像行星），玩家必須橫越整個場地逼近她。
       *
       * 刻意不再鋪滿全場：危險只長在軌道上，軌道之間是空的，
       * 所以這關考的是「抓住兩顆球之間的縫隙、一圈一圈往內跳」，
       * 而不是原本那種在滿場毒場裡找唯一一條安全路徑。
       */
      /** 最內圈半徑（要大於她的體型，否則球會埋在她身上） */
      innerRadius: 150,
      /** 相鄰兩圈的間距——必須小於斬擊最遠距離，玩家才跳得過去 */
      ringGap: 175,
      /** 圈數 */
      ringCount: 5,
      /**
       * 同一圈上相鄰兩顆球的弧長間距。越大越好穿越；
       * 球數由半徑自動換算（`2πr / arcSpacing`），所以外圈自然比內圈多球。
       */
      arcSpacing: 150,
      /** 每圈至少幾顆球，免得最內圈只剩兩三顆形同虛設 */
      minPerRing: 5,
      /** 每幾顆球有一顆是可以斬來回體力的特殊球 */
      specialEvery: 3,
      ...
      innerSpeed: 0.85,
      /**
       * 每往外一圈，角速度乘上這個倍率——外圈轉得慢，就像外側行星週期長。
       * 視覺上會看到整個系統在「慢慢轉開」，而不是鐵板一塊一起轉。
       */
      speedFalloff: 0.82,
      /** 相鄰圈反向公轉，穿越時每一層的縫隙都在往不同方向跑 */
      alternateDirection: true,

      /** 玩家起點周圍這個半徑內不生成球——被推到牆邊時不會一睜眼就貼臉 */
      safeRadius: 190,

      /** 傷害球碰到玩家的傷害 */
      ballDamage: 18,
      /**
       * 這一關的體力球回復量，**刻意低於一般的 `special.staminaRestore`(42)**。
       * 給滿的話玩家可以無腦一路滿力斬衝到底（實測 5 秒就結束），
       * 這一關的重點就沒了——要的是「體力永遠差一點，必須繞去撿球」。
       */
      staminaPerBall: 24,
